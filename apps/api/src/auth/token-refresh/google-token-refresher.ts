...
@Injectable()
export class GoogleTokenRefresher implements OAuthTokenRefresher {
  readonly provider = ExternalProvider.GOOGLE;
  ...
  async refresh(refreshToken: string): Promise<RefreshedTokens> {
    ...
    if (!response.ok) {
      throw new Error(`Falha ao renovar token Google: HTTP ${response.status}`);
    }
